...
Since the surface of new risks that come with developing appchains is vast, we cannot possibly fit everything into a single post. Thus, this article will be focused only on the security surface of the core chain. We are reserving CosmWasm and IBC-related risks for a series of future posts.
...
> **Note:** while non-determinism should generally be avoided, we have provided a list in the appendix covering exactly which parts of the Cosmos SDK where the code _needs_ to be deterministic. In general, anything that touches the state machine must be.
...
Network requests are generally non-deterministic. As a result, they should be avoided in the state machine. If anything related to API calls are implemented, then determinism must be a first-order consideration that is concretely addressed. 

For instance, an in-protocol price oracle that requires validators to directly query centralized exchanges for prices might choose to only push the median price into state machine scope.

### Concurrency And Multithreading

Thread or goroutine pre-emption is likely to lack determinism. As a result, one should generally be careful when using goroutines to be used anywhere within the state-machine scope. There are, of course, exceptions where we may process data concurrently for aggregation/counting which would be deterministic.

For more complex operations, a viable approach to do parallelism safely is to have each thread operate on a separate `CacheKV` store followed by sequential `CacheKV.Write()`s. These scenarios should still be implemented with great care, with particular attention dedicated to guaranteeing that there are no write conflicts between indepedent `CacheKV` stores. In any case, concurrency and multithreading on the same KV store poses a serious safety hazard.

### Cross-Platform Floats

For reasons that could easily take up a [separate article](https://randomascii.wordpress.com/2013/07/16/floating-point-determinism/), it is safe to claim that native Go float arithmetic is non-deterministic across platforms. Therefore, they must never be used in the app chain state-machine scope. Use fixed point arithmetic if you need to process decimal numbers.

### Diverging Gas Totals

Non-determinism can also come from different numbers of state reads during a transaction, as the gas totals would be different between nodes. It is important to ensure that there are no superfluous non-deterministic reads.
...
Thus, **we should be cognizant of when we use panics and ensure that we avoid them with behavior that could be handled well with an error.** Of course, it is still okay to guardrail unexpected flows with panics when needed, especially if the behavior is such that a chain halt _would_ be appropriate.
...
By default, all SDK math operations [panic on overflows](https://github.com/cosmos/cosmos-sdk/blob/eaf92c225cfa2420946ff738cdcf05cf8b402040/math/dec.go#L278). This means that any math that is done in functions that get called in `Begin/EndBlock` should make sure to catch overflow panics using a helper similar to the one linked above.
...
This is a consideration that seems trivial but comes up much more frequently than one might expect. If a loop in unmetered code is never exited or a recursion base case is never hit, it can lead to an expensive chain halt.
...
While this might seem trivial in simple cases, there are many scenarios where pricing state writes is not as straightforward. This complexity generally surfaces in actions that create externalities for other users.

For instance, if the protocol includes logic where an arbitrary set of tokens need to consistently be iterated through linearly, then each new token can potentially push an increasing cost onto the system. In such cases, additional (and scaling) gas [must be charged](https://github.com/osmosis-labs/osmosis/blob/b0aee0006ce55d0851773084bd7880db7e32ad70/x/tokenfactory/keeper/createdenom.go#L19-L22) to ensure the protocol is not vulnerable to DoS attacks.

### Fees on Contract-called Functions

Charging fixed fees distinct from gas is a relatively common design pattern (for instance, Osmosis charges a governance-set fee denominated in OSMO for creating pools). However, introducing such fees often exposes new risks in scenarios where contracts act on behalf of users, especially if the contract does not have white-glove logic to bubble up the fee to specific users. In these cases, such design patterns can cause the fees to be charged incorrectly or even prevent the contract from being used at all.

If possible, it often makes sense to simply [charge the fee as additional gas](https://github.com/celestiaorg/celestia-app/blob/6f19fd83c4d0061c717a40e23a368a2f60aff715/x/blob/keeper/keeper.go#L53-L54) so that it gets floated up to the caller. If this is not an option due to the fee being too high, then the fee charge needs to be factored into the design of the contract(s) triggering it.

### Broken or Missing Fee Market

During periods of high network usage, it is critical to ensure that the chain remains live and usable through a robust fee market mechanism. While there are many viable approaches to this, [EIP-1559](https://www.youtube.com/watch?v=62UI3Js30Io) has proven to be a particularly effective default. [Osmosis recently implemented](https://github.com/osmosis-labs/osmosis/blob/b0aee0006ce55d0851773084bd7880db7e32ad70/x/txfees/keeper/mempool-1559) this directly in the mempool, achieving the desired outcome without requiring a chain upgrade/hard fork. 

However, the superior long-term solution is to integrate the fee market directly into consensus by leveraging `ABCI 2.0`. The Skip team has been spearheading [the implementation](https://github.com/skip-mev/feemarket) of this initiative which will be released as a pluggable component that appchains can integrate.
...
While this does not affect chain security, it can cause severe client instability until clients update their gas estimation logic. It would also increase chain load stemming from clients retrying their failed transactions. If your appchain supports high-value and time-sensitive transactions such as ones related to collateral and leverage, these issues could be quite problematic.
